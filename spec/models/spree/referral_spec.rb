...
describe Spree::Referral, :type => :model do
  ...
  context "with user" do
    before(:each) do
      @user = create(:user)
      @referred = create(:user, referral_code: @user.referral.code)
      @order = create(:order, user: @referred)
      @referred_no_purchase = create(:user, referral_code: @user.referral.code)
    end
    ...
    it "returns an array of referral activated users with completed_orders" do
      referred_user_with_completed_order = create(:user, referral_code: @user.referral.code)
      completed_order = create(:order, completed_at: @user.created_at + 1.second, user: referred_user_with_completed_order)
